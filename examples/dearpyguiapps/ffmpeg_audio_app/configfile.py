...
class JsonConfig:
    ...
    def __getattr__(self, item):
        """allows for getting config values by accessing an attribute"""
...
FFMPEG_DEFAULT = 'DIRECTORY/FFMPEG.exe'

cfg = JsonConfig(
    Path('config.json'),
    ffmpeg_executable=FFMPEG_DEFAULT
)


def validate_ffmpeg_executable():
    try:
        return Path(cfg.ffmpeg_executable).exists()
    except:
        return False
